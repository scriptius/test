...
Подобного рода задачи были рреализованы мной при написании учебного framrwork, по ссылке
...
3. Объект класса DB необходимо вынести в отдельный класс и сделать его singleton, чтобы не создавать дубликатов соединений к БД и быть уверенным, что мы работаем с нужной базой.
4. Нет подготовленных запросов, что может приветсти к SQL-инъекциям или XSS атакам.
5. Рекомендуется инициализировать первоначальные значения переменных, например $data в load_users_data(). Что предотвратит ненужные возможные ошибки, которые сложно отследить.
6. Пароль к БД хранится не в конфиге, причем в открытом виде. Что крайне небезопасно. Да и пароль не сильно сложный :). Я бы сделал конфиг, в котором сделал бы секцию db и при деплое при помощи phing подменял {{USERNAME}} и {{PASSWORD}} на конкретные значения
```php
 'db' => [
    'host'     => 'localhost',
    'dbName'   => 'test',
